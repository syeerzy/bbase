...
(module oop (class?
                 ...
                 (define-class
                   ...
                   oop:id-subset?
                   oop:build-application)
                 oop:dispatch-table   ; sigh, currently used by src/swl/base2.ss
                 <base>)
...
(ctdef oop:id-intersect (oop:id-filter (lambda (is1 is2) (oop:id-list-index (car is2) is1))))

(ctdef oop:id-setrember (oop:id-filter (lambda (is1 is2) (not (oop:id-list-index (car is2) is1)))))
...
;      (if (null? is2)
;          is1
;          (if (not (oop:id-list-index (car is2) is1))
;              (cons (car is2) (lp (cdr is2)))
;              (lp (cdr is2)))))))

(ctdef oop:id-subset?
  (lambda (is1 is2)
    (andmap (lambda (x) (oop:id-list-index x is2)) is1)))
...
;;         for the subclass) --- oop:dispatch-table can weed out duplicates
...
(define-syntax oop:build-application
  (lambda (form)
    (syntax-case form ()
      ((_ rator rand ...)
       (syntax (rator rand ...)))
...
;; having to do the oop:id-union here.
;; Note that instead of actually doing an oop:id-union we figure out which
...
;; just once.
                  (with-syntax ((new-prot (oop:id-setrember
                                             (syntax (prot-name ...))
                                             (syntax (prot ^...)))))
                 ...
                 ((_ 'make-instance residual (e ^...))
                  (syntax (lambda mfmls (oop:build-application
                 ...
                 ((_ make-subclass (scname . _mfmls) _mactls
                  ...
                  (let ((base-inhtbl (syntax (inhtbl ...)))
                    ...
                    (issue-syntax-error-msgs
                      ((not (oop:id-subset? sub-inhtd base-inhtbl))
                       (warningf 'define-class
                         "~s attempts to inherit uninheritable ivars ~s from ~s"
                         (syntax-object->datum (syntax scname))
                         (syntax-object->datum (oop:id-setrember base-inhtbl sub-inhtd))
                         (syntax-object->datum (syntax name))))
                      ((not (null? (oop:id-intersect sub-inhtd sub-ivars)))
                       (warningf 'define-class
                         "inherited ivars ~s overlap ivars"
                         (syntax-object->datum (oop:id-intersect sub-inhtd sub-ivars))))
                      ((not (oop:id-subset? sub-inhtbl (append sub-ivars sub-inhtd)))
                       (warningf 'define-class
...
;; compute these sets via mrvs
             [new-prot
              (oop:id-setrember (syntax (prot ...)) (syntax (_prot-name ...)))]
             [redef-prot
              (oop:id-intersect (syntax (prot ...)) (syntax (_prot-name ...)))])
#;       (when  oop:define-class-verbose
         ...
         (let ([ivar-len (length base-ivars)]
               ...
               [ihtd-prot (oop:id-setrember redef-prot base-prots)])
           (with-syntax ([set! (datum->syntax-object (syntax scname) 'set!)]
                         ...
                         [(redef-prot-index ...)
                          (map (lambda (x)
                                 (+ protected-start-index
                                    (or (oop:id-list-index x base-prots)
                         ...
                         [(ihtd-prot-index ...)
                          (map (lambda (x)
                                 (+ protected-start-index
                                    (oop:id-list-index x base-prots)))
                               ihtd-prot)]
                         [(privtmp ...)
                         ...
                         [(ihtd-index ...)
                          (map (lambda (x)
                                 (+ ivar-start-index
                                    (or (oop:id-list-index x base-ivars)
